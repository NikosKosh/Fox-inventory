...
def record_blocked_login_attempt(request, username, reason):
    ...
    recent_since = timezone.now() - timedelta(seconds=settings.LOGIN_BLOCKED_LOG_INTERVAL_SECONDS)
    duplicate = LoginAttempt.objects.filter(
        username_normalized=normalize_login_username(username), ip_address=ip,
        result=LoginAttempt.Result.BLOCKED, created_at__gte=recent_since,
    ).exists()
    if duplicate:
        return None
    return record_login_attempt(request, username, LoginAttempt.Result.BLOCKED, reason=reason)
...
def _username_ip_failures(username_normalized, ip, now):
    ...
    last_success = LoginAttempt.objects.filter(
        username_normalized=username_normalized, ip_address=ip,
        result=LoginAttempt.Result.SUCCESS, created_at__gte=window_start,
    ...
    stats = LoginAttempt.objects.filter(
        username_normalized=username_normalized, ip_address=ip,
        result=LoginAttempt.Result.FAILED, created_at__gt=since,
...
def _ip_failures(ip, now):
    ...
    window_start = now - timedelta(minutes=settings.LOGIN_FAILURE_WINDOW_MINUTES)
    stats = LoginAttempt.objects.filter(
        ip_address=ip, result=LoginAttempt.Result.FAILED, created_at__gte=window_start,
    ).aggregate(count=Count("pk"), latest=Max("created_at"))
    return stats["count"], stats["latest"]
...
def get_login_lockout(username, ip, now=None):
    ...
    if not active:
        return LoginLockout(username_failures=username_count, ip_failures=ip_count)
    until, scope = max(active, key=lambda item: item[0])
    return LoginLockout(blocked=True, until=until, scope=scope,
                        username_failures=username_count, ip_failures=ip_count)
