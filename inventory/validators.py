...
BUSINESS_DOCUMENT_EXTENSIONS = {".pdf", ".jpg", ".jpeg", ".png", ".doc", ".docx", ".xls", ".xlsx", ".csv", ".xml", ".rtf", ".txt", ".zip", ".7z"}
...
def validate_business_document(file):
    ext = Path(file.name).suffix.lower()
    if ext not in BUSINESS_DOCUMENT_EXTENSIONS:
        raise ValidationError(
            "Разрешены PDF, изображения, DOC/DOCX, XLS/XLSX, CSV, XML, RTF, TXT, ZIP и 7Z."
        )
    limit = settings.MAX_UPLOAD_MB * 1024 * 1024
    if file.size > limit:
        raise ValidationError(f"Файл больше {settings.MAX_UPLOAD_MB} МБ.")
