...
- Добавлена единая учётная цена на номенклатуру. Изменение цены в одном месте применяется ко всем связанным экземплярам.
- Миграция `0017_catalog_pricing` автоматически создаёт номенклатуру для существующих карточек и сохраняет все текущие привязки оборудования, сотрудников, объектов, SN/MAC и историю движений.
- Миграция использует артикулы из модели, когда они доступны: например, Honor `5301ARGN` и `5301ARGQ` остаются разными товарами, а косметические варианты Logitech M170 с одним артикулом объединяются.
- Исторические закупочные цены из комментариев импорта переносятся в историю цен. При нескольких исторических ценах текущая цена выбирается автоматически по последней доступной дате и помечается как требующая подтверждения.
- Новая отличающаяся закупочная цена больше не перезаписывает подтверждённую единую учётную цену молча: она записывается в историю и отправляет номенклатуру на проверку.
- Добавлены страницы «Номенклатура» и карточка модели с количеством экземпляров, общей стоимостью, историей цен и быстрым подтверждением цены.
- Новые карточки оборудования и импорт автоматически привязываются к существующей номенклатуре либо создают новую, предотвращая повторное расхождение названий одинаковых устройств.
- В карточках оборудования, на складе, в общем списке и поиске отображается каноническое название номенклатуры и единая цена.
- Добавлена стоимость оборудования на главную панель и контроль отсутствующих/неподтверждённых цен.
- В XLSX-отчёт добавлены цена и стоимость, а сверху выводится общая учётная стоимость активного оборудования.
- Новые акты получают неизменяемый снимок строк с названием, моделью, внутренним номером, SN, количеством, ценой и суммой на момент создания.
- DOCX акта теперь содержит отдельные колонки «Цена за ед.» и «Сумма», а также итоговую стоимость имущества.
- Акт возврата повторно использует цену из последнего акта выдачи этому сотруднику, если такой снимок существует.
- После создания ценового снимка состав оборудования в акте блокируется от изменения при обычном редактировании, чтобы документ и исторические данные не расходились.
- Существующие 42 production-акта не переписываются и не получают искусственно рассчитанные цены задним числом; их сохранённые документы остаются неизменными.
- Добавлен безопасный `scripts/verify-1.6.0.sh` для пост-релизной проверки связи всех экземпляров с номенклатурой, разделения ARGN/ARGQ, цен, миграции и health-check.

## 1.5.1 — 2026-08-14

- Полностью переработан XLSX-экспорт оборудования: вместо 22 технических колонок основной лист содержит 10 читаемых пользовательских полей.
- В верхней части отчёта добавлена сводка по позициям, единицам и основным статусам оборудования.
- На основном листе объединены модель и серийный номер, а сотрудник/объект/помещение/шкаф собраны в одно понятное поле размещения.
- Технические поля MAC, Hostname, адрес управления и логин вынесены на отдельный лист «Технические данные».
- Сырые комментарии и исходные поля размещения вынесены на отдельный лист «Служебные данные» и больше не растягивают основную таблицу.
- Добавлены фиксированные разумные ширины колонок, перенос текста, закрепление заголовков, автофильтры, печать в альбомной ориентации и визуальное выделение статусов/состояния.
- Добавлены регрессионные тесты структуры XLSX-отчёта и разделения пользовательских, технических и служебных данных.

## 1.5.0 — 2026-08-12

- Ограничено раскрытие паролей сетевых устройств: доступ только для staff-пользователей.
- Запрещено архивирование сотрудника, пока за ним закреплено оборудование или активная временная передача.
- Запрещено архивирование оборудования в активных рабочих состояниях.
- Временная передача другой организации теперь сохраняет исходное состояние оборудования и восстанавливает его при возврате.
- Добавлена миграция `0016_equipmentloan_previous_state` для хранения состояния до временной передачи.
- Для сломанного оборудования форма не позволяет сохранить обычный рабочий статус вместо ремонта/списания.
- Возврат из карточки больше не хранится в общей session, поэтому вкладки не перетирают друг другу маршрут назад.
- На мобильных устройствах восстановлен быстрый доступ к глобальному поиску, а список сотрудников адаптирован без горизонтальной таблицы на 940 px.
- Основные действия в карточках сотрудника и оборудования упрощены; вторичные операции перенесены в меню «Ещё».
- Быстрый просмотр оборудования отменяет предыдущий запрос при быстром переключении карточек.
- В документообороте убран дублирующий блок контекста выбранных сторон.
- На объекте больше не скрываются документы после первых 30 записей.
- Восстановлены недостающие в handoff исходники исторических миграций и защиты входа, необходимые для полноценной чистой установки.
